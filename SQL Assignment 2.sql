...
SELECT Emp_ID FROM Employee_Details WHERE Emp_ID LIKE 'A%A';
...
SELECT * FROM payment_details WHERE PAYMENT_STATUS = 'NOT PAID';
...
SELECT PAYMENT_MODE, ROUND((COUNT(PAYMENT_MODE) / @total_count) * 100,2) AS Percentage_Contribution FROM Payment_Details GROUP BY PAYMENT_MODE;
...
UPDATE logistics_Emp SET TOTAL_PAYABLE_CHARGES = Sd_charges + Amount;
...
SELECT Cust_ID, Cust_name, Start_Date, End_Date, ROUND(DATEDIFF(End_Date, Start_Date)/365,0) 
	AS Membership_Years FROM logistics_Emp HAVING Membership_Years > 10;


# 10) Who got the product delivered on the next day the product was sent?
SELECT * FROM logistics_Emp HAVING DELIVERY_DATE-SENT_DATE = 1;
...
SELECT Sd_content, SUM(AMOUNT) AS Content_Wise_Amount FROM logistics_Emp GROUP BY (Sd_content) ORDER BY Content_Wise_Amount DESC LIMIT 5;

# 12) Which product categories from shipment content are transferred more?  
SELECT Sd_content, COUNT(Sd_content) AS Content_Wise_Count FROM logistics_Emp GROUP BY(Sd_content) ORDER BY Content_Wise_Count DESC LIMIT 5;

# 13) Create a new view 'TXLogistics' where employee branch is Texas.
CREATE VIEW TXLogistics AS SELECT * FROM logistics_Emp WHERE Emp_branch = 'TX';
...
ALTER VIEW TXLogistics AS SELECT *, Amount - ((Amount * 5)/100) AS New_Price FROM logistics_Emp WHERE Emp_branch = 'TX';
SELECT * FROM TXLogistics;
...
UPDATE logistics_Emp SET Emp_branch = 'NJ' WHERE Emp_branch = 'NY';
...
SELECT Cust_type, (COUNT(Cust_type)/@total_count)*100 AS Contribution FROM logistics_Emp GROUP BY Cust_type;

# 19) Rename the column SER_TYPE to SERVICE_TYPE.
ALTER TABLE logistics_Emp CHANGE SER_TYPE SERVICE_TYPE VARCHAR (15);

# 20) Which service type is preferred more?
SELECT SERVICE_TYPE, COUNT(SERVICE_TYPE) AS Frequency FROM logistics_Emp GROUP BY SERVICE_TYPE ORDER BY Frequency DESC;

# 21) Find the shipment id and shipment content where the weight is greater than the average weight.
SELECT Sd_id, Sd_content, Sd_weight FROM Shipment_Details WHERE Sd_weight > (SELECT AVG(Sd_weight) FROM Shipment_Details);